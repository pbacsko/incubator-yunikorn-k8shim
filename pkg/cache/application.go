...
func (app *Application) setContext(ctx *Context) {
	// set during registration only, before the app is visible to other goroutines
	app.context = ctx
}
...
func (app *Application) removeFromSchedulerCore() {
	log.Log(log.ShimCacheApplication).Info("removing application from scheduler core",
		zap.String("appID", app.applicationID))
	if err := app.schedulerAPI.UpdateApplication(
		common.CreateUpdateRequestForRemoveApplication(app.applicationID, app.partition)); err != nil {
